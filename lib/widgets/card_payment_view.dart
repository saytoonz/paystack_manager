...
class _CardPaymentViewState extends State<CardPaymentView> {
  ...
  void initState() {
    ...
    _cardNumberTextEditingController.text =
        widget.paymentInfo.paymentCard?.number ?? "";

    _cardCVVTextEditingController.text =
        widget.paymentInfo.paymentCard?.cvv ?? "";

    _cardExpiryTextEditingController.text =
        "${widget.paymentInfo.paymentCard?.month ?? ""}/${widget.paymentInfo.paymentCard?.year ?? ""}";
  }
    ...
    return ListView(
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: Center(
            child: Text(
              "Verify your card details to pay",
              style: TextStyle(
